...
function updateUI() {
    ...
    if (currentUser) {
        ...
        toggleAdminPanel();
        updateHeaderAvatar();
        // Загружаем квесты при обновлении UI
...
document.getElementById('finishBtn').addEventListener('click', function() {
    ...
    if (challenge && !currentUser.dailyChallengeDone) {
        ...
        // ============================================================
        //  ИНТЕГРАЦИЯ С КВЕСТАМИ (ЧЕЛЛЕНДЖ)
        // ============================================================
    ...
    // ============================================================
    //  ИНТЕГРАЦИЯ С КВЕСТАМИ (ТРЕНИРОВКА)
    // ============================================================
    if (typeof updateQuestProgress === 'function') {
    ...
    loadAchievements();
    loadChart();
    // Обновляем квесты после тренировки
    if (typeof loadDailyQuests === 'function') {
        loadDailyQuests();
    }
});
    ...
    CONFIG.ACHIEVEMENTS.forEach(ach => {
        ...
        switch(ach.type) {
            ...
            case 'all_titles':
                ...
                break;
            case 'quests':                     // 👈 НОВОЕ
                val = currentUser.totalQuestsCompleted || 0;
                break;
            default: return;
        }
...
function getDailyChallenge() {
    ...
    if (!ch || ch.date !== today) {
        ...
        if (typeof sendNotification === 'function') {
            sendNotification('🔥 Новый челлендж!', `Сегодняшний челлендж: ${picked.task}`);
        }
    }
    return ch;
}
...
function renderAdminAnalytics() {
    ...
    const totalQuestsCompleted = users.reduce((sum, u) => sum + (u.totalQuestsCompleted || 0), 0);  // 👈 НОВОЕ
    container.innerHTML = `
        <h5>Аналитика:</h5>
        <div style="font-size:13px; line-height:1.6;">
            ...
            <p>📜 Всего выполненных квестов: <strong>${totalQuestsCompleted}</strong></p>   <!-- 👈 НОВОЕ -->
        </div>
    `;
}
...
document.querySelectorAll('.tab-btn').forEach(btn => {
    btn.addEventListener('click', function() {
        ...
        document.querySelectorAll('#trainTab,#shopTab,#inventoryTab,#simpleTasksTab,#leaderTab,#chartTab,#fightTab,#achievementsTab,#settingsTab,#questsTab').forEach(el => el.style.display = 'none');
