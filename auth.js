...
//  + Уведомления, реферальная ссылка из URL, копирование ссылки
// ================================================================

document.addEventListener('DOMContentLoaded', function () {

    // ================================================================
    // 1. РЕФЕРАЛЬНАЯ ССЫЛКА ИЗ URL
    // ================================================================
    ...
    if (refParam) {
        ...
        if (refInput) {
            ...
            console.log('🔗 Реферальный код из URL:', refParam);
        }
    }
    ...
    function migrateUser(user) {
        ...
        // Daily Quests – НОВОЕ
        // ------------------------------------------------------------
        if (!user.dailyQuests || typeof user.dailyQuests !== 'object' || Array.isArray(user.dailyQuests)) {
            user.dailyQuests = {};
        }
    ...
    if (loginBtn) {
        loginBtn.addEventListener(
            ...
            async function () {
                ...
                // ============================================================
                //  ЗАПРОС РАЗРЕШЕНИЯ НА УВЕДОМЛЕНИЯ (НОВОЕ)
                // ============================================================
        ...
        registerSubmitBtn.addEventListener(
            ...
            async function () {
                ...
                const newUser = {
                    ...
                    customExercises: [],
                    achievements: {},
                    dailyQuests: {}   // НОВОЕ — поддержка ежедневных квестов
                };
    ...
    // ================================================================
    //  НОВАЯ ФУНКЦИЯ: КОПИРОВАНИЕ РЕФЕРАЛЬНОЙ ССЫЛКИ
    // ================================================================

    function copyReferralLink() {
        if (!currentUser) {
            showToast('Войдите в аккаунт, чтобы скопировать ссылку', 'error');
            return;
        }
        const link = window.location.origin + '/fitness/?ref=' + currentUser.nickname;
