// ================================================================
//  KWON FITNESS – ИГРОВАЯ МЕХАНИКА
//  Магазин, инвентарь, боссы, промокоды, простые задания, лидерборд
// ================================================================
...
function buyItem(itemId) {
    ...
    // Снимаем очки и добавляем предмет
...
function completeSimpleTask(taskId, reward) {
    if (!currentUser) {
        showToast(t('alert_no_user'), 'error');
        return;
    }
...
function loadBoss() {
    ...
    // Генерируем нового босса на сегодня
    ...
    // Сохраняем данные босса для обработчика
    window._bossData = bossData;
}

// Обработчик боя с боссом
...
document.addEventListener('click', function(e) {
    ...
    if (useBtn) {
        const itemId = useBtn.dataset.id;
        const type = useBtn.dataset.type;
        useItem(itemId, type);
        return;
    }

    // Кнопка выполнения простого задания
    ...
    if (simpleBtn) {
        ...
        completeSimpleTask(taskId, reward);
        return;
    }

    // Клик по имени в лидерборде
