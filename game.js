// ================================================================
//  KWON FITNESS – ИГРОВАЯ МЕХАНИКА (обновлённая)
//  Магазин, инвентарь, боссы, промокоды, простые задания с таймером
// ================================================================
...
function buyItem(itemId) {
    if (!currentUser) {
        showToast(t('alert_no_user'), 'error');
        return;
    }
    ...
    if (isItemOwned(currentUser, itemId)) {
        showToast('Уже куплено', 'error');
        return;
    }

    if (currentUser.points < item.price) {
        showToast(t('alert_no_points'), 'error');
        return;
    }
...
function loadInventory() {
        ...
        items.forEach(item => {
            ...
            div.innerHTML = `
                <div class="item-info">
                    ...
                    <div>
                        <div><strong>${item.name}</strong></div>
                        <div style="font-size:11px;color:var(--text-secondary);">${t('type_' + type)}</div>
                    </div>
                </div>
                <div class="item-actions">
                    ${isActive ? 
    ...
    if (inv.length === 0) {
        container.innerHTML = '<p style="color:var(--text-secondary); text-align:center;">Нет предметов</p>';
    }
}
...
function useItem(itemId, type) {
    if (!currentUser) return;
    if (!isItemOwned(currentUser, itemId)) {
        showToast('Предмет не куплен', 'error');
        return;
    }
...
function loadSimpleTasks() {
    ...
    const availableTasks = CONFIG.SIMPLE_TASKS.filter(task => age >= task.ageMin && age <= task.ageMax);

    if (availableTasks.length === 0) {
        container.innerHTML = '<p style="color:var(--text-secondary); text-align:center;">Нет доступных заданий</p>';
        return;
    }
        ...
        div.innerHTML = `
            <div class="item-info">
                ...
                <div>
                    ...
                    <div class="item-type">+${task.reward} монеты</div>
                </div>
            </div>
            <div class="item-actions">
                <button class="btn btn-sm simple-task-btn" data-id="${task.id}" data-reward="${task.reward}">
...
document.addEventListener('click', function(e) {
    const btn = e.target.closest('.simple-task-btn');
    if (!btn) return;

    const taskId = btn.dataset.id;
    const reward = parseInt(btn.dataset.reward) || 2;
    startSimpleTask(btn, taskId, reward);
});

function startSimpleTask(btn, taskId, reward) {
    if (btn.disabled) return; // уже выполняется

    btn.disabled = true;
    const originalText = btn.innerHTML;

    let remaining = 10; // 10 секунд
    btn.innerHTML = `<i class="fas fa-spinner fa-spin"></i> ${remaining}...`;

    const interval = setInterval(() => {
        remaining--;
        if (remaining <= 0) {
            clearInterval(interval);
            // Начисляем монеты в try-catch, чтобы даже при ошибке кнопка восстановилась
            try {
                if (currentUser) {
                    ...
                    showToast(`+${reward} монеты!`, 'success');
                    playSound('buy');
                    updateRankDisplay();
                    updateAchievements();
                }
    ...
    if (simpleTaskTimers[taskId]) clearInterval(simpleTaskTimers[taskId]);
    simpleTaskTimers[taskId] = interval;
}
...
document.getElementById('fightBtn').addEventListener('click', function() {
    ...
    if (!boss || boss.fought) {
        showToast(t('fight_already'), 'error');
        return;
    }
...
document.getElementById('activatePromoBtn').addEventListener('click', function() {
    ...
    if (!currentUser) {
        result.textContent = t('alert_no_user');
        return;
    }
    ...
    if (promo.used >= promo.maxUses) {
        result.textContent = 'Код исчерпан';
        return;
    }
    if (currentUser.usedPromoCodes?.includes(code)) {
...
function loadLeaderboard() {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        item.className = 'leaderboard-item skeleton-item';
        item.innerHTML = `
    ...
    // Заполняем реальными данными через 600 мс
    setTimeout(() => {
...
function renderLeaderboardList(list) {
    ...
    sorted.slice(0, 10).forEach((u, i) => {
        ...
        div.innerHTML = `
            <span>#${i + 1} <span class="leader-name" data-nick="${u.nickname}" style="cursor:pointer; color:var(--kwon-purple-light); text-decoration:underline;">${u.nickname}</span></span>
    ...
    if (sorted.length === 0) {
        list.innerHTML = '<p style="color:var(--text-secondary);">' + t('no_data') + '</p>';
    }
}
...
document.addEventListener('click', function(e) {
    ...
    if (buyBtn) {
        buyItem(buyBtn.dataset.id);
        return;
    }

    const useBtn = e.target.closest('.use-btn');
    if (useBtn) {
        useItem(useBtn.dataset.id, useBtn.dataset.type);
        return;
    }

    const simpleBtn = e.target.closest('.simple-task-btn');
    if (simpleBtn) {
        const taskId = simpleBtn.dataset.id;
        const reward = parseInt(simpleBtn.dataset.reward) || 2;
        startSimpleTask(simpleBtn, taskId, reward);
        return;
    }

    const leaderName = e.target.closest('.leader-name');
    if (leaderName) {
        const nick = leaderName.dataset.nick;
        const users = getUsers();
