...
function requestNotificationPermission() {
    ...
    Notification.requestPermission().then(function(permission) {
        if (permission === 'granted') {
            ...
            sendNotification('🔔 Уведомления включены', 'KWON Fitness будет присылать важные события');
        }
    }).catch(function(err) {
        console.warn('Ошибка запроса уведомлений:', err);
    });
}

function sendNotification(title, body) {
    // Проверка настроек (чекбокс в настройках)
