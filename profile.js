...
function getFrameColor(frameId) {
    ...
    // Сначала проверяем кастомные изображения из CONFIG
    if (
...
function isFrameImage(frame) {

    return (
        typeof frame === 'string' &&
        (
            /^(https?:|data:|blob:|\/|\.{1,2}\/)/.test(frame) ||
            /\.(png|jpg|jpeg|webp|gif|svg)(\?.*)?$/i.test(frame)
        ) &&
        !frame.includes('gradient') &&
        !frame.startsWith('#') &&
        !frame.startsWith('rgb')
    );
}


function resetFrameStyles(element) {
...
function applyFrameStyle(element, frame, borderWidth) {
    if (!element) return;
    resetFrameStyles(element);

    const width = borderWidth || 3;

    // ------------------------------------------------------------
    // Кастомная рамка-картинка
    // ------------------------------------------------------------
if (isFrameImage(frame)) {
    element.style.backgroundImage = `url("${frame}")`;
    element.style.backgroundSize = 'contain';   // ← главное изменение
    element.style.backgroundPosition = 'center';
    element.style.backgroundRepeat = 'no-repeat';
    element.style.border = 'none';
    // Добавляем цвет фона, чтобы не было пустых мест
    element.style.backgroundColor = 'transparent';
    element.style.boxShadow = borderWidth >= 4
        ? '0 0 25px rgba(108,92,231,.85)'
    ...
    // Градиентная рамка
    // ------------------------------------------------------------
    if (typeof frame === 'string' && frame.includes('gradient')) {
        element.style.border = `${width}px solid transparent`;
        element.style.backgroundImage = `
            linear-gradient(var(--kwon-black), var(--kwon-black)),
            ${frame}
        `;
    ...
    // ------------------------------------------------------------
    // Обычная цветная рамка
    // ------------------------------------------------------------
    element.style.border = `${width}px solid ${frame || '#6c5ce7'}`;
    element.style.boxShadow = borderWidth >= 4
        ? `0 0 25px ${frame || '#6c5ce7'}`
        : '0 0 15px rgba(108,92,231,.6)';
}
    ...
    if (!items.length) {
        ...
        items.forEach(
            item => {
                ...
                // Для кастомной рамки показываем изображение,
                // а для обычных предметов — emoji
                const frameImage =
                    type === 'frame' &&
                    typeof CONFIG !== 'undefined' &&
                    CONFIG.CUSTOM_DESIGNS &&
                    CONFIG.CUSTOM_DESIGNS.frames &&
                    CONFIG.CUSTOM_DESIGNS.frames[item.id];
