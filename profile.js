...
//  Обновлено: усилена рамка аватара, добавлено свечение
// ================================================================
...
function updateHeaderAvatar() {
    ...
    // Применяем рамку (увеличена до 3px по запросу)
    const frameItem = findItemById(currentUser.frame);
    if (frameItem) {
        avatarEl.style.border = '3px solid ' + getFrameColor(frameItem.id); // FIXED: было 2px
    } else {
...
function openProfile(user, isSelf = false) {
    ...
    // Баннер (высота 160px задаётся в CSS)
    ...
    // Рамка аватара (толщина 4px)
    const frameColor = getFrameColor(user.frame);
    avatarEl.style.border = `4px solid ${frameColor}`; // FIXED: было 4px? подтверждаем

    // NEW: Усиленное свечение аватара в цвет рамки
    if (typeof frameColor === 'string' && !frameColor.includes('gradient')) {
        avatarEl.style.boxShadow = `0 0 20px ${frameColor}`;
    } else {
        // Для градиентных рамок используем фиолетовый (можно заменить на нейтральный)
        avatarEl.style.boxShadow = '0 0 20px var(--kwon-purple)';
    }
    ...
    if (isSelf) {
        actions.innerHTML = `
            <button class="btn btn-sm" onclick="openAvatarPicker()"><i class="fas fa-user-circle"></i> ${t('change_avatar_btn')}</button>
            <button class="btn btn-sm" onclick="openItemPicker('frame')"><i class="fas fa-border-all"></i> ${t('change_frame')}</button>
            <button class="btn btn-sm" onclick="openItemPicker('banner')"><i class="fas fa-image"></i> ${t('change_banner')}</button>
            <button class="btn btn-sm" onclick="openItemPicker('title')"><i class="fas fa-tag"></i> ${t('change_title')}</button>
        `;
    } else {
        actions.innerHTML = '';
    }
...
// ---------- Универсальная функция выбора предмета (рамка, баннер, титул) ----------
function openItemPicker(type) {
    const modal = document.getElementById('avatarPickerModal');
    const list = document.getElementById('avatarPickerList');
    list.innerHTML = '';

    const items = currentUser.inventory.filter(i => i.type === type);
    const typeNames = {
        avatar: 'аватар',
        frame: 'рамку',
        banner: 'баннер',
    ...
    if (items.length === 0) {
        list.innerHTML = `<p style="color:var(--text-secondary);">Нет доступных ${typeNames[type] || 'предметов'}</p>`;
    } else {
        items.forEach(item => {
            const div = document.createElement('div');
            div.className = 'avatar-picker-item';
            ...
            div.onclick = () => {
                setActiveItem(currentUser, type, item.id);
                saveCurrentUser();
                updateHeaderAvatar();
                updateUI();
                closeAvatarPicker();
    ...
    if (titleEl) titleEl.textContent = t('item_picker_title', { type: typeNames[type] || 'предмет' });
    modal.style.display = 'flex';
}
